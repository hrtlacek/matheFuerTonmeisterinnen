...
Alternativ wird manchmal nie \emph{normalisierte \index{sinc Funktion}} verwendet in der  Faktor $\pi$ in die Funktion hineingenommen wird: $sin(\pi x) \ \pi x$.

Die Funktion hat für uns relevanz u.a. da sie die \index{Impulsantwort} eines idealen Lowpass \index{Filter}s beschreibt\citep{enwiki:sincFilter}. Eine möglichkeit sich diese Tatsache zu verdeutlichen ist dass die $sinc$ Funktion aus Cusinus schwingungen unterschiedlicher Frequenzen zusammengesetzt werden kann, siehe Abb \ref{fig:sincAsSum}. 
...
Für die Tontechnik und Signalverarbeitung gibt es hier viele Anwendungen, seien es Filter, Verzerrungen etc. Oft müssen wir \emph{\index{Nullstellen}} (Engl. 'roots') oder \emph{\index{Polstellen}} einer Funktion finden. 
...
Da man in der Gestaltung von Filtern, Hall Algorithmen etc oft die Notwendigkeit hat Funktionen zu Gestalten und deren Nullstellen an bestimmte Orte zu verschieben ist es notwendig, nicht nur ein Polynom einem Computer zuzuführen um die Nullstellen 'ausgespuckt' zu bekommen. Es ist oft Notwendig Systeme umzuformulieren um 'Zugriff' auf die Orte der Nullstellen zu haben, zb. durch Zerlegung in Linearfaktoren. 
...
Wenn wir ein wenig über die Hintergründe von digitalen \index{Filter} recherchieren stoßen wir schnell auf 
\cite{FILTERSWEB07}\footnote{\href{http://ccrma.stanford.edu/\~jos/filters/BiQuad\_Section.html}{http://ccrma.stanford.edu/\~jos/filters/BiQuad\_Section.html}}. Eines der Standardwerke wenn es um digitale Signalverarbeitung im musikalischen Kontext geht. Hier finden wir einen Filter der sich '\index{Biquad}'\footnote{Der Name 'Biquad' deutet darauf hin dass hier 2 ('Bi') quadratische Gleichungen vorliegen.} nennt und durch Gleichung \ref{eq:biquad} beschrieben wird.
...
f(x) = \frac{P(x)}{Q(x)}
\]
mit \(\deg(P) < \deg(Q)\), zerlegt man \(f(x)\) in die Form 
\[
f(x) = \sum_{i} \frac{A_i}{(x - r_i)^{m_i}} + \sum_{j} \frac{B_j x + C_j}{(x^2 + px + q)^{n_j}}.
\]
Hierbei sind \(r_i\) reelle Nullstellen und \(x^2 + px + q\) irreduzible quadratische Faktoren von \(Q(x)\).
...
\begin{itemize}
	\item Gratis
	\item Open Source
	\item General Purpose Langage (Industriestandard fuer Machine Learnung zB.) 
\end{itemize}
...
Beispielsweise das Polynom $f(x) = x^2 - 6^x + 9$ liefert das \texttt{Dictionary} \texttt{\{3: 2\}}. 
