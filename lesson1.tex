...
\begin{itemize}
    \item $D$ \index{Definitionsbereich}, \index{Definitionsmenge}
    \item $Z$ \index{Zielmenge}
    \item $f(A)$ \index{Bildmenge} (wobei stets $f(A)=B, B \in Z$, heißt: Die Bildmenge ist eine Teilmenge der Zielmenge.)
    \item $x$ unabhängige Variable
    \item $y$ abhängige Variable
    \item $f(x) = y$ \index{Funktionsgleichung}
\end{itemize}
